...
public interface CourseRepository extends MongoRepository<Course, String> {
    ...
    @Query("{'$and': [{'price': {$gte: ?1, $lte: ?2}}, {'$or': [{'nameNormalized': {$regex: ?0, $options: 'i'}}, {'descriptionNormalized': {$regex: ?0, $options: 'i'}}]}]}")
    ...
    @Query("{'$and': [{'price': {$gte: ?2, $lte: ?3}}, {'teacherId': {$in: ?1}}, {'$or': [{'nameNormalized': {$regex: ?0, $options: 'i'}}, {'descriptionNormalized': {$regex: ?0, $options: 'i'}}]}]}")
    ...
    @Query("{'$or': [{'nameNormalized': {$regex: ?0, $options: 'i'}}, {'descriptionNormalized': {$regex: ?0, $options: 'i'}}]}")
    List<Course> findCoursesByKeyword(String keyword);

    @Query("{'$and': [{'teacherId': {$in: ?1}}, {'$or': [{'nameNormalized': {$regex: ?0, $options: 'i'}}, {'descriptionNormalized': {$regex: ?0, $options: 'i'}}]}]}")
