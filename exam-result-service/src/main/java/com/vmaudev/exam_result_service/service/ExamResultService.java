...
public class ExamResultService {
    ...
    public ExamResult createExamResult(String assignmentId, String userId, List<String> userAnswers) {
        ...
        AssignmentResponse assignment = assignmentClient.getAssignmentById(assignmentId, token);
        ...
        // Tính tổng điểm trên thang điểm 10
        int totalQuestions = questionResults.size();
        int correctAnswers = (int) questionResults.stream().filter(ExamResult.QuestionResult::isCorrect).count();
        int score = totalQuestions > 0 ? (int) Math.round((double) correctAnswers / totalQuestions * 10) : 0;
